...
';
    ...
    public function getHaproxyConfig()
    {
         ...
         */
        $httpsAfterBind = $httpsMode == 'tcp'
            ? ''
            : 'ssl crt-list /etc/haproxy/haproxy-crt-list';
    ...
    # http-response set-header Strict-Transport-Security max-age=15768000 # only http mode';
        ...
        foreach ($sitesServers as $sitesServersGrouped) {
            ...
            if ($httpsMode == 'tcp') {
                ...
                if ($split[$site->id]['cdn']) {
                    $config .= "\n" . '    use_backend backend-dynamic-' . $site->id . ' if { ssl_fc_sni ' . $split['nonCdn'][$site->id] . ' }';
                    $config .= "\n" . '    use_backend backend-static-' . $site->id . ' if { ssl_fc_sni ' . $split['cdn'][$site->id] . ' }';
                } else {
                    $config .= "\n" . '    use_backend backend-dynamic-' . $site->id . ' if { ssl_fc_sni ' . $split['all'][$site->id] . ' }';
                }
            }
        }
        /**
        ...
        foreach ($sitesServers as $sitesServersGrouped) {
            ...
            foreach ($workers as $worker) {
                $allWorkers[$worker->id] = $worker;
                $workerHttpsPort = $httpsMode == 'tcp'
                    ? $worker->getSettingValue('service.nginx.httpsPort', 8084)
